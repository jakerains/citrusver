  ...
  async check() {
    try {
      ...
      if (cachedData && !this.shouldCheckAgain(cachedData.lastCheck)) {
        // Re-validate cached data against current version (user may have updated)
        const stillNeedsUpdate = this.compareVersions(cachedData.latestVersion, this.currentVersion) > 0;
        return stillNeedsUpdate ? { ...cachedData, updateAvailable: true } : null;
      }
