...
class BranchProtection {
  ...
  isBranchUpToDate() {
    try {
      ...
      const remotes = execSync('git remote 2>/dev/null', { encoding: 'utf8', shell: true }).trim();
      if (!remotes) {
        // No remote configured, treat as up-to-date
        return { isUpToDate: true, status: 'no-remote' };
      }
      
      // Fetch latest from remote if it exists
      try {
        execSync('git fetch --quiet 2>/dev/null', { stdio: 'pipe', shell: true });
      } catch (e) {
        // Fetch failed, but continue anyway
      }
      ...
      try {
        ...
        // No commits yet, so we're "up to date"
        return { isUpToDate: true, status: 'no-commits' };
      }
      
      let remoteCommit;
      try {
        remoteCommit = execSync(`git rev-parse origin/${currentBranch} 2>/dev/null`, { 
          encoding: 'utf8',
          shell: true
        }).trim();
      } catch (e) {
      ...
      try {
        ahead = parseInt(execSync(`git rev-list --count origin/${currentBranch}..HEAD 2>/dev/null`, {
          encoding: 'utf8',
          shell: true
        }).trim());
        
        behind = parseInt(execSync(`git rev-list --count HEAD..origin/${currentBranch} 2>/dev/null`, {
          encoding: 'utf8',
          shell: true
        }).trim());
      } catch (e) {
        // If we can't compare with remote, treat as up-to-date
        return { isUpToDate: true, status: 'no-remote' };
      }
