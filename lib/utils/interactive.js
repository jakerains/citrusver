...
class InteractiveMode {
  ...
  async promptForCustomVersion(currentVersion) {
    const rl = readline.createInterface({
      input: process.stdin,
      output: process.stdout
    });
    ...
    return new Promise((resolve, reject) => {
      ...
      const prompt = `${colors.cyan}Enter new version:${colors.reset} `;

      rl.question(prompt, (answer) => {
        rl.close();
        const version = answer.trim();
        if (!version) {
          reject(new Error('Version is required'));
        } else {
          resolve(version);
        }
      });
  ...
  async promptForLabel() {
    ...
    return new Promise((resolve) => {
      const prompt = `\n${colors.cyan}Enter label${colors.reset} ${colors.gray}(e.g., elf, rc, canary):${colors.reset} `;
