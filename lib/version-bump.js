...
class CitrusVer {
  ...
  async bump(versionType, options = {}) {
    ...
    // Handle custom version type - prompt or validate provided version
    if (versionType === 'custom') {
      if (!options.customVersion) {
        if (options.quiet) {
          console.error('❌ Custom version required in quiet mode. Usage: citrusver custom 1.2.3 --quiet');
          process.exit(1);
        }
        ...
        options.customVersion = await this.interactive.promptForCustomVersion(currentPackageJson.version);
      }
      // Validate the custom version
      if (!this.versionStrategies.validateVersion(options.customVersion)) {
        console.error(`❌ Invalid version format: ${options.customVersion}`);
        process.exit(1);
      }
    }

    // Handle label flag - prompt if flag is true (no inline value)
    if (options.label === true) {
      if (options.quiet) {
        console.error('❌ Label value required in quiet mode. Usage: citrusver patch --label myLabel --quiet');
        process.exit(1);
      }
      options.label = await this.interactive.promptForLabel();
    }
  ...
  async bumpFull(versionType, options = {}) {
    ...
    try {
      ...
      if (options.label && typeof options.label === 'string') {
        newVersion = this.versionStrategies.applyLabel(newVersion, options.label);
      }
  ...
  async bumpVersionOnly(versionType, options = {}) {
    ...
    if (options.dryRun) {
      ...
      if (options.label) {
        console.log(`  • Label: ${colors.magenta}${options.label}${colors.reset}`);
      }
