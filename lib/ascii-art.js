...
function citrusHeader(version = '') {
  const versionLine = version ? `                      ${colors.cyan}v${version}${colors.reset}\n` : '';
  return `

                    ${colors.brightYellow}🍋 ${colors.bright}CITRUSVER${colors.reset} ${colors.brightYellow}🍋${colors.reset}
${versionLine}              ${colors.gray}Interactive Version Management${colors.reset}
`;
}
...
function createReleaseBox(type, currentVersion, nextVersion) {
  ...
  const typeColors = {
    ...
    minor: colors.cyan,
    major: colors.green,
    custom: colors.magenta,
    prerelease: colors.brightCyan
  };

  const color = typeColors[type] || colors.yellow;
...
function showSuccess(version, options = {}) {
  const { committed = false, tagged = false, pushed = false } = options;

  let statusLines = '';
  if (committed) {
    statusLines += `            ${colors.gray}Changes have been committed${colors.reset}\n`;
  }
  if (tagged) {
    statusLines += `               ${colors.gray}Git tag has been created${colors.reset}\n`;
  }
  if (pushed) {
    statusLines += `               ${colors.gray}Changes pushed to remote${colors.reset}\n`;
  }

  // If no git operations, show file update message
  if (!committed && !tagged && !pushed) {
    statusLines = `            ${colors.gray}package.json has been updated${colors.reset}\n`;
  }
